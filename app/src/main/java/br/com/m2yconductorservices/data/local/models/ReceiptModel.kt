...
fun TransferResellerResponse.toReceiptModel(): ReceiptModel {
    return ReceiptModel(
        id?.toString(),
        transactionCode,
        transactionDate?.m2yCdtChangeDateFormat(M2YCDTConstants.CDT_DATE_FORMAT, M2YCDTConstants.RECEIPT_DATE_FORMAT),
        ReceiptType.TRANSFER,
        transfer = ReceiptTransferModel(
            "Conta $destinationAccount", amount, null, null,
            null, null, null, TransferType.RESSELER
        ),
...
//            "\n\n${this.nameFavored}"
    return ReceiptModel(
        id?.toString(),
        this.transactionCode,
        transferenceDate?.m2yCdtChangeDateFormat(M2YCDTConstants.CDT_DATE_FORMAT, M2YCDTConstants.RECEIPT_DATE_FORMAT),
        ReceiptType.TRANSFER,
        transfer = ReceiptTransferModel(
            idOriginAccount?.toInt().toString(),
            value,
            nameFavored,
            cpfCnpjFavored.toString(),
            bankName,
            agencyFavored?.toInt().toString(),
            "${accountFavored?.toInt()}-${digitAccountFavored}",
            TransferType.BANK
        )
    )
}
...
fun Transferp2pResponse.toReceiptModel(): ReceiptModel {
    return ReceiptModel(
        id?.toString(), this.transactionCode,
        date?.m2yCdtChangeDateFormat(M2YCDTConstants.CDT_DATE_FORMAT, M2YCDTConstants.RECEIPT_DATE_FORMAT),
        ReceiptType.TRANSFER,
        description = description,
        transfer = ReceiptTransferModel(
            originalAccount?.toInt().toString(),
            amount,
...
fun RechargeVoucherResponse.toReceiptModel(): ReceiptModel {
    ...
    val recharge = ReceiptRechargeModel(
        phoneNumber = "($ddd) $phone",
        operator = dealerName,
        name = nickname,
        value = amountFloat.m2yCdtFormatCurrencyBRL()
    )

    return ReceiptModel(
        orderId,
        transactionCode,
        rechargeDate.m2yCdtChangeDateFormat(M2YCDTConstants.RECHARGE_DATE_FORMAT, M2YCDTConstants.RECEIPT_DATE_FORMAT),
        ReceiptType.RECHARGE,
        recharge = recharge
    )
}